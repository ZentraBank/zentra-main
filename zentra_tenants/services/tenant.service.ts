...
type TenantApiResponse = {
  ...
  features?: Record<string, { enabled: boolean; configuration?: unknown }>;
};

export async function getCurrentTenant(): Promise<Tenant> {
  const response = await api.get<{ data: TenantApiResponse }>("/tenants/current");
  ...
  return {
    ...
    tenant_features: Object.entries(data.features || {}).map(([key, feature]) => ({
      key,
