...
export default function AddClientPage() {
  ...
  const handleSave = async () => {
    const names = client.name.trim().split(/\s+/).filter(Boolean);
    if (names.length < 2) {
      setSaveError("Enter the client's first and last name.");
      return;
    }

    if (!client.email.trim()) {
      setSaveError("Enter the client's email address.");
      return;
    }

    const firstName = names[0];
    const lastName = names[names.length - 1];
    const middleName = names.length > 2 ? names.slice(1, -1).join(" ") : undefined;
    ...
    try {
      const result = await createClient({
        ...
        account: {
          accountName: client.name.trim(),
          accountType: client.accountType.toLowerCase() as "savings" | "current" | "investment",
          currency: "USD",
        },
      });
      ...
      if (result.account?.account_number) {
        update("accountNumber", result.account.account_number);
      }
    } catch (error) {
        ...
        <div className="min-h-screen bg-black">
          <header className="sticky top-0 z-40 border-b border-white/10 bg-black/95 px-4 py-4 backdrop-blur md:px-8">
            <div className="mx-auto flex max-w-7xl items-center justify-between">
              ...
              >
                <Save size={15} />
                <span className="hidden md:inline">{saving ? "Saving..." : "Save Client"}</span>
              </button>
            </div>
          </header>
          ...
          <div className="mx-auto grid max-w-[430px] gap-4 px-3 py-4 md:max-w-7xl md:grid-cols-[390px_1fr] md:px-8 md:py-8 xl:grid-cols-[430px_1fr]">
            <aside className="space-y-4 md:sticky md:top-24 md:self-start">
              ...
              <section className="hidden rounded-3xl border border-white/10 bg-white/[0.04] p-5 shadow-2xl md:block">
                ...
                >
                  {saving ? "Creating Client..." : "Create Client Profile"}
                </button>
                ...
                {createdCredentials && (
                  <div className="mt-3 rounded-xl bg-emerald-500/15 px-3 py-3 text-xs text-emerald-100">
                    ...
                    {createdCredentials.accountNumber && (
                      <p>Account: {createdCredentials.accountNumber}</p>
                    )}
                ...
                <Card title="Account Information">
                  ...
                  <Row label="Account Type">
                    <AdminSelect
                      ...
                      options={["Savings", "Current", "Investment"]}
                    />
                  </Row>
