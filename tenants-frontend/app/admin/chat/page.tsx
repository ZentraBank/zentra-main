...
export default function AdminChatPage() {
  ...
  const insertSmartReply = () => {
    setInput(
      "I understand your concern, Michael. I’ll check the transaction status immediately and keep you updated until this is fully resolved."
    );
  };
      ...
      <div className="mx-auto flex min-h-screen max-w-[430px] flex-col border-x border-white/10 px-4 pb-3 pt-11 md:max-w-[1100px] md:px-8">
        <header className="mb-4 flex items-center gap-3 md:mb-8">
          ...
          <div>
            ...
            <p className="text-[10px] text-green-400 md:text-sm">Online now</p>
          </div>
        </header>
            ...
            {messages.map((message, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`max-w-[245px] rounded-[20px] px-3 py-2 text-[12px] leading-[1.25] md:max-w-[520px] md:px-5 md:py-4 md:text-base ${
                      isAdmin
        ...
        <footer className="mt-2 flex items-center gap-2 rounded-full bg-[linear-gradient(135deg,#ffffff,#5aa2ff,#1c4be8)] p-1 md:mx-auto md:mt-5 md:w-full md:max-w-[780px]">
          <button
            ...
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white text-[#1d4ed8] md:h-11 md:w-11"
          >
            <Sparkles size={18} />
          </button>
          ...
          <button
            type="button"
            onClick={sendMessage}
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#2458ff] text-white md:h-11 md:w-11"
          >
            <Send size={17} />
          </button>
        </footer>
