...
// Mock data extracted from the render cycle. 
// Replace this with actual API data later.
const RECENT_TRANSACTIONS = [
  { id: "tx-1", title: "Account credited", amount: "+₦50,000", type: "in" },
  { id: "tx-2", title: "Transfer sent", amount: "-₦5,000", type: "out" },
];
...
export default function DashboardPage() {
  return (
    <AppShell>
      ...
      style={{
        background:
          "linear-gradient(135deg, var(--tenant-primary, #2563eb), #111827)",
      }}
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        <QuickCard title="Add Client" href="/clients/add" />
        <QuickCard title="View Transactions" href="/transactions" />
        <QuickCard title="Open Chat" href="/chat" />
      </div>
...
function QuickCard({ title, href }: { title: string; href: string }) {
  return (
    <Link
      href={href}
      className="flex items-center justify-between rounded-2xl border bg-white p-5 shadow-sm hover:shadow-md transition-shadow"
    >
