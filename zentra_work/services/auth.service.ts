...
export const authService = {
  ...
  resendRegistration(email: string): Promise<CodeResponse> {
    return apiRequest("/auth/register/resend", { method: "POST", skipAuth: true, body: JSON.stringify({ email }) });
  },

  forgotPassword(email: string): Promise<CodeResponse> {
    return apiRequest("/auth/forgot-password", { method: "POST", skipAuth: true, body: JSON.stringify({ email }) });
  },

  resetPassword(email: string, code: string, newPassword: string): Promise<null> {
    return apiRequest("/auth/reset-password", { method: "POST", skipAuth: true, body: JSON.stringify({ email, code, newPassword }) });
  },
  ...
  socialLoginUrl(provider: SocialProvider, next = "/dashboard"): string {
